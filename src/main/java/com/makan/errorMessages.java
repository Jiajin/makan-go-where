package com.makan;

public class errorMessages {
    public static final String PersonNotFound = "The user trying to create the session does not exist";
    public static final String MeetingNotFound = "The makan session does not exist";
    public static final String RecordNotFound = "The specified record does not exist";
    public static final String NotMeetingHost = "Only the makan host can end the session";
    public static final String MeetingOver = "The makan location has been chosen!";
    public static final String NoMeetingLocations = "There are no submitted locations for this makan session";
}
